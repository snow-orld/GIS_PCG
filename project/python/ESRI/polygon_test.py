...
date: 2018/4/9
version: 1.0.1
modified: 2018/4/10 15:01:00 +0800

developing env: python 3.5.2
dependencies: polygon, random, math
...
def operationsExample():
	...
	writeSVG('test.svg', plist, width=800, labels = slist, labels_centered=True)
...
def moonExample():
	print('### Moon')
	# a hight-resolution, softly flickering moon,
	...
	d = ['outside', 'inside']
	c = moon.center()
	print('Did you know that the center of graviation of my moon is %s?' %d[moon.isInside(c[0], c[1])])
...
def sierpinskiExample():
	...
	for l in range(7):
		...
		print('SIERPINSKI CARPET - Level: %2d - Contours: %dd - Area: %g' % (l, len(s), s.area()))
